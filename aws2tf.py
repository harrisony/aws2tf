...
def dd_threaded(ti):
    if not context.rproc[ti]:
        ...
        context.tracking_message = (
            "Stage 5 of 10, Processing Dependancy, " + str(i) + " " + str(id)
        )
...
def validate_resource_id(resource_id: str, resource_type: str = None) -> str:
    ...
    for char in dangerous_chars:
        if char in resource_id:
            raise ValueError(
                f"Invalid character '{char}' in resource ID: {resource_id}"
            )
...
def parse_and_validate_arguments():
    ...
    argParser.add_argument(
        "-l", "--list", help="List extra help information", action="store_true"
    )
    ...
    argParser.add_argument(
        "-f", "--fast", help="fast multi-threaded mode", action="store_true"
    )
    argParser.add_argument(
        "-v", "--validate", help="validate and exit", action="store_true"
    )
    argParser.add_argument(
        "-w", "--warn", help="show warning messages", action="store_true"
    )
    argParser.add_argument(
        "--status", help="show STATUS messages every 20 seconds", action="store_true"
    )
    argParser.add_argument(
        "-a", "--accept", help="expected plan changes accepted", action="store_true"
    )
...
def setup_environment_and_context(args):
    ...
    if args.type is None or args.type == "":
        if args.serverless:
            log.error(
                "type is required eg:  -t aws_vpc  when in serverless mode, exiting ...."
            )
            log.info("exit 003")
            timed_interrupt.stop_timer()
            exit()
...
def check_terraform_version(timed_interrupt):
    ...
    if tvmaj < 1:
        log.error(
            "Terraform version is too old - please upgrade to v1.9.5 or later "
            + str(tv)
        )
        timed_interrupt.stop_timer()
        sys.exit(1)
    ...
    try:
        if args.profile is None:
            ...
            boto3.setup_default_session(
                region_name=region, profile_name=context.profile
            )
    except Exception as e:
...
def get_aws_account(timed_interrupt):
    ...
    try:
        ...
        if "ExpiredToken" in str(e):
            ...
            log.error(
                "Failed to connect to AWS - check network connectivity, exiting ....."
            )
        else:
            log.error(str(e))
            log.error(str(exn))
...
def setup_workspace(args, region):
    ...
    if args.serverless:
        ...
        context.path1 = (
            "/tmp/aws2tf/generated/tf-" + context.pathadd + context.acc + "-" + region
        )
    ...
    if not os.path.isfile(context.path1 + "/terraform.tfstate") and context.merge:
        log.warning(
            "No terraform.tfstate file found in " + context.path1 + " - can not merge"
        )
        context.merge = False
...
def process_stack_type(id, timed_interrupt):
    ...
    if id is None:
        ...
        context.tracking_message = (
            "Stage 3 of 10 getting stack " + id + " resources ..."
        )
...
def process_multiple_resource_types(all_types, id):
    ...
    if context.fast:
        ...
        context.tracking_message = (
            "Stage 3 of 10 getting " + str(it) + " resources multi-threaded"
        )
        ...
        with ThreadPoolExecutor(max_workers=context.cores) as executor:
            futures = [executor.submit(common.call_resource, i, id) for i in all_types]
            for future in tqdm(
        ...
        for i in tqdm(
            ...
            context.tracking_message = (
                ...
                + " of "
                + str(it)
                + " resource types \t currently getting "
                + i
            )
...
def process_known_dependencies():
    ...
    if context.fast:
        context.tracking_message = (
            "Stage 4 of 10, Known Dependancies - Multi Threaded " + str(context.cores)
        )
        with ThreadPoolExecutor(max_workers=context.cores) as executor12:
            futures2 = [executor12.submit(kd_threaded(ti)) for ti in list(context.rdep)]
    else:
...
def process_detected_dependencies():
    ...
    while detdep:
        ...
        unprocessed_deps = [
            ti for ti in list(context.rproc) if not context.rproc.get(ti, True)
        ]
        ...
        if context.fast:
            context.tracking_message = (
                "Stage 5 of 10, Detected Dependancies: Multi Threaded "
                + str(context.cores)
            )
            ...
            if total_deps > 0:
                ...
                with ThreadPoolExecutor(max_workers=context.cores) as executor2:
                    futures = [
                        executor2.submit(dd_threaded(ti)) for ti in unprocessed_deps
                    ]
            ...
            if total_deps > 0:
                log.info(
                    f"Processing {total_deps} new detected dependencies loop "
                    + str(lc + 1)
                )
                for ti in tqdm(
                    unprocessed_deps,
                    ...
                    id = ti.split(".", 1)[1]
                    log.debug(
                        "DD calling getresource with type=" + i + " id=" + str(id)
                    )
                    common.call_resource(i, id)
        ...
        context.tracking_message = "Stage 6 of 10, Dependancies Detection: Loop " + str(
            lc
        )
        x = glob.glob("import__aws_*.tf")
        context.esttime = len(x) / 4
        ...
        if len(x) > 0:
            for fil in tqdm(
                x, desc=f"Moving files (loop {lc})", unit="file", leave=False
            ):
                tf = fil.split("__", 1)[1]
                try:
        ...
        context.tracking_message = (
            "Stage 6 of 10, Dependancies Detection: Loop " + str(lc) + " terraform plan"
        )
        common.tfplan1("loop " + str(lc))
        context.tracking_message = (
            "Stage 6 of 10, Dependancies Detection: Loop "
            + str(lc)
            + " fixing tf files"
        )
        log.info(
        ...
        context.tracking_message = (
            "Stage 6 of 10, Completed " + str(lc) + " dependancy check loops"
        )
            ...
            for ti in context.rproc.keys():
                if not context.rproc[ti]:
                    log.error(
                        "ERROR: Not found "
                        + str(ti)
                        + " - check if this resource still exists in AWS. Also check what resource is using it - grep the *.tf files in the generated/tf.* subdirectory"
                    )
                    context.tracking_message = (
                        "No change/progress in dependancies exiting..."
                    )
...
def finalize_and_cleanup(args, starttime):
    ...
    if awsf > 0:
        ...
        log.warning(
            "Please review these error files and report via github issue if needed"
        )
