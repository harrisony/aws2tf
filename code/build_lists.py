...
def build_lists():
    ...
    def fetch_s3_data():
        try:
            ...
            return {"resource_type": "s3", "items": validated_items, "metadata": {}}
        except Exception as e:
    ...
    def fetch_roles_data():
        try:
            client = boto3.client(
                "iam", region_name="us-east-1", config=BOTO3_RETRY_CONFIG
            )
            response = []
    ...
    def fetch_instprof_data():
        try:
            ...
            return {
                "resource_type": "inp",
                "items": [
                    {"id": j["InstanceProfileName"], "data": j} for j in response
                ],
                "metadata": {},
            }
        except Exception as e:
    ...
    def _write_resource_files(results):
        ...
        for result in results:
            if result.get("resource_type") == "subnet":
                files_to_write["imported/subnets.json"] = result.get(
                    "metadata", {}
                ).get("full_data")
            elif result.get("resource_type") == "iam":
                files_to_write["imported/roles.json"] = result.get("metadata", {}).get(
                    "full_data"
                )
        ...
        with concurrent.futures.ThreadPoolExecutor(
            ...
            for future in tqdm(
                ...
                try:
                    ...
                    context.attached_role_policies_list[role_name] = result[
                        "attached_policies"
                    ]
