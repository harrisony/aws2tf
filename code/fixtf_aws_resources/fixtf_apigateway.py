...
def aws_api_gateway_method(t1, tt1, tt2, flag1, flag2):
    ...
    if tt1 == "resource_id" and tt2 != "null":
        t1 = (
            tt1
            + " = aws_api_gateway_resource.r-"
            + str(context.apigwrestapiid)
            + "_"
            + tt2
            + ".id\n"
        )
        common.add_dependancy(
            "aws_api_gateway_resource", str(context.apigwrestapiid) + "_" + tt2
        )
    if tt1 == "authorizer_id" and tt2 != "null":
        t1 = (
            tt1
            + " = aws_api_gateway_authorizer.r-"
            + str(context.apigwrestapiid)
            + "_"
            + tt2
            + ".id\n"
        ...
        common.add_dependancy(
            "aws_api_gateway_authorizer", str(context.apigwrestapiid) + "_" + tt2
        )
...
log.debug(
    f"APIGATEWAY handlers: 3 custom functions + __getattr__ for 0 simple resources"
)
...
def aws_api_gateway_usage_plan_key(t1, tt1, tt2, flag1, flag2):
    ...
    if tt1 == "usage_plan_id" and tt2 != "null":
        t1 = (
            tt1
            + " = aws_api_gateway_usage_plan.r-"
            + str(context.usageplanid)
            + ".id\n"
        )
...
def aws_api_gateway_method_settings(t1, tt1, tt2, flag1, flag2):
    ...
    if tt1 == "stage_name" and tt2 != "null":
        t1 = (
            tt1
            + " = aws_api_gateway_stage.r-"
            + str(context.apigwrestapiid)
            + "-"
            + str(context.apigwstagename)
            + ".stage_name\n"
        )
        common.add_dependancy(
            "aws_api_gateway_stage",
            str(context.apigwrestapiid) + "/" + str(context.apigwstagename),
        )
    return skip, t1, flag1, flag2
