...
def aws_autoscaling_group(t1, tt1, tt2, flag1, flag2):
    ...
    if tt1 == "capacity_rebalance":
    ...
    elif tt1 == "load_balancers" and tt2 == "[]":
        skip = 1
    elif tt1 == "target_group_arns":
        skip = 1
    elif tt1 == "availability_zones":
        if len(tt2) > 4:
            context.asg_azs = True
        ...
        tt1 == "force_delete"
        or tt1 == "force_delete_warm_pool"
        or tt1 == "ignore_failed_scaling_activities"
    ):
        if tt2 == "null":
            t1 = tt1 + " = false\n"
...
def aws_launch_configuration(t1, tt1, tt2, flag1, flag2):
    ...
    if tt1 == "id":
        ...
        if len(resp["LaunchConfigurations"]) > 1:
            log.warning(
                "WARNING Got >1 launch configuations in fixtf_autoscaling aws_launch_configuration"
            )
        try:
...
log.debug(
    f"AUTOSCALING handlers: 2 custom functions + __getattr__ for 0 simple resources"
)
