...
def aws_secretsmanager_secret_version(t1, tt1, tt2, flag1, flag2):
    ...
    try:
        if t1.startswith("resource"):
            vid = t1.split("_")[-1]
            vid = (
                vid.replace('"', "").replace("{", "").replace(" ", "").replace("\n", "")
            )
            context.secvid = vid
        elif tt1 == "secret_id":
            context.secid = tt2
            ...
            if "null" in tt2:
                ...
                response = client.get_secret_value(
                    SecretId=context.secid, VersionId=context.secvid
                )
...
log.debug(
    f"SECRETSMANAGER handlers: 3 custom functions + __getattr__ for 0 simple resources"
)
