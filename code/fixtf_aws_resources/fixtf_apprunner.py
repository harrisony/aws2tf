...
def aws_apprunner_service(t1, tt1, tt2, flag1, flag2):
    ...
    if tt1 == "auto_scaling_configuration_arn":
        if "autoscalingconfiguration/DefaultConfiguration/1" in tt2:
            skip = 1
    if tt1 == "image_identifier":
        ...
        if tt2.startswith(context.acc) and context.region in tt2:
            ...
            t1 = (
                tt1
                + ' = format("%s.dkr.ecr.%s.amazonaws.com/%s",data.aws_caller_identity.current.account_id,data.aws_region.current.region,"'
                + backend
                + '")\n'
            )
...
log.debug(
    f"APPRUNNER handlers: 1 custom functions + __getattr__ for 0 simple resources"
)
