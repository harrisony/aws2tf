"""
EC2 Resource Handlers - Optimized with __getattr__

This file contains ONLY EC2 resources with custom transformation logic.
All other EC2 resources automatically use the default handler via __getattr__.

Original: 128 functions
Optimized: 24 functions + __getattr__
...
# ============================================================================
# EC2 Resources with Custom Logic (24 functions)
# ============================================================================
...
def aws_ebs_volume(t1, tt1, tt2, flag1, flag2):
    ...
    if "throughput" in tt1:
        if tt2 == "0":
            skip = 1
    elif tt1 == "volume_initialization_rate" and tt2 == "0":
        skip = 1
...
def aws_eip_association(t1, tt1, tt2, flag1, flag2):
    ...
    if tt1 == "instance_id":
        ...
        skip = 1
    elif tt1 == "private_ip_address":
        skip = 1
    elif tt1 == "network_interface_id":
        skip = 1
    ...
    try:
        if tt1 == "ipv6_addresses":
            ...
            resp = client.describe_instance_attribute(
                Attribute="userData", InstanceId=inid
            )
            try:
                ...
                with open(flag2 + ".sh", "w") as f:
                    f.write(ud2)
                t1 = (
                    'user_data_base64 = filebase64sha256("'
                    + flag2
...
def aws_launch_template(t1, tt1, tt2, flag1, flag2):
    ...
    if tt1 == "security_group_names":
        ...
        if tt2 == "0":
            skip = 1
    elif tt1 == "name_prefix":
        skip = 1
    elif tt1 == "http_put_response_hop_limit":
        if tt2 == "0":
            skip = 1
    elif tt1 == "excluded_instance_types" and tt2 == "[]":
        skip = 1
    elif tt1 == "included_instance_types" and tt2 == "[]":
        skip = 1
    elif tt1 == "volume_initialization_rate" and tt2 == "0":
        skip = 1
    elif tt1 == "on_demand_max_price_percentage_over_lowest_price" and tt2 == "0":
        skip = 1
    elif tt1 == "spot_max_price_percentage_over_lowest_price" and tt2 == "0":
        skip = 1
    elif (
        tt1 == "max_spot_price_as_percentage_of_optimal_on_demand_price" and tt2 == "0"
    ):
        skip = 1
    return skip, t1, flag1, flag2
...
def aws_nat_gateway(t1, tt1, tt2, flag1, flag2):
    ...
    if "secondary_private_ip_address_count" in tt1:
        if tt2 == "0":
            skip = 1
    elif tt1 == "regional_nat_gateway_address":
        skip = 1
    elif tt1 == "private_ip":
        skip = 1
    elif tt1 == "public_ip":
...
def aws_default_route_table(t1, tt1, tt2, flag1, flag2):
    skip = 0
    return skip, t1, flag1, flag2
...
def aws_spot_fleet_request(t1, tt1, tt2, flag1, flag2):
    skip = 0
    if tt1 == "load_balancers" and tt2 == "null":
        skip = 1
    elif tt1 == "target_group_arns" and tt2 == "null":
        skip = 1
...
def aws_vpc_endpoint_service(t1, tt1, tt2, flag1, flag2):
    skip = 0
    if tt1 == "gateway_load_balancer_arns" and tt2 == "[]":
        skip = 1
    elif tt1 == "network_load_balancer_arns" and tt2 == "[]":
        skip = 1
    elif tt1 == "allowed_principal" and tt2 == "[]":
        skip = 1
...
def aws_vpn_connection(t1, tt1, tt2, flag1, flag2):
    skip = 0
    if tt1.startswith("tunnel") and tt2 == "0":
        skip = 1
    return skip, t1, flag1, flag2
...
def aws_verifiedaccess_trust_provider(t1, tt1, tt2, flag1, flag2):
    ...
    if tt1 == "client_secret":
        if "null" in tt2:
            t1 = (
                tt1
                + ' = "PLACEHOLDER_UPDATE_MANUALLY" # TODO: Update with actual secret\n'
            )
            common.log_warning(
                "WARNING: client_secret for %s must be manually updated", flag2
            )
    return skip, t1, flag1, flag2
...
def __getattr__(name):
    ...
    This allows getattr(module, 'aws_ami') to work even if aws_ami function
    doesn't exist, by returning the default handler.

    All simple EC2 resources (104 resources) automatically use this:
    aws_ami, aws_ami_copy, aws_ami_from_instance, aws_customer_gateway,
    aws_flow_log, aws_internet_gateway, aws_volume_attachment, etc.
    """
