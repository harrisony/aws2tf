...
def fixtf(ttft, tf):
    ...
    callfn = "fixtf_" + clfn
    if context.debug:
        log.debug("callfn=" + callfn + " ttft=" + ttft)
    ...
    if ttft == "aws_s3_bucket_replication_configuration":
        for t1 in Lines:
            t1 = t1.strip()
            if context.debug5:
                log.debug("DEBUG5: pre scan block1 : t1=%s", t1)
            skip = 0
            ...
            if tt1 == "bucket":
                try:
                    ...
                    if "arn:aws:s3" in tt2:
                        ...
                        if context.debug5:
                            log.debug(
                                "DEBUG5: pre scan block 2: common.add_dep bucket_name=%s",
                                tt2,
                            )
    ...
    if ttft == "aws_lb":
        for t1 in Lines:
            ...
            if tt1 == "access_logs":
                accessl = 1
                cnxl = 0
            if tt1 == "connection_logs":
                accessl = 0
                cnxl = 1
            if tt1 == "enabled":
                if tt2 == "false" and accessl == 1:
                    context.lbskipaacl = True
                if tt2 == "false" and cnxl == 1:
                    context.lbskipcnxl = True
                else:
                    context.lbenabled = True
    ...
    with open(tf2, "w") as f2:
        ...
        for t1 in Lines:
            ...
            try:
                ...
                skip, t1, flag1, flag2 = aws_common.aws_common(
                    ttft, t1, tt1, tt2, flag1, flag2
                )
                ...
                if context.stripblock != "":
                    if context.stripblock in t1:
                        context.lbc = 1
                    elif context.stripstart in t1 and context.lbc > 0:
                ...
                log.error(
                    "-- no fixtf for "
                    + tf
                    + " calling generic fixtf2.aws_resource callfn="
                    + callfn
                )
                log.error("t1=" + str(t1))
                nofind = 2
        ...
        if nofind > 0:
            log.warning(
                "WARNING: No fixtf for "
                + tf
                + " calling generic fixtf2.aws_resource nofind="
                + str(nofind)
            )
...
def globals_replace(t1, tt1, tt2):
    ...
    if tt2.startswith("[") and tt1 != "managed_policy_arns" and "," in tt2:
        ...
        arns = tt2.split(",")
        if context.debug:
            log.debug("Globals replace an array:" + str(arns))
        fins = ""
        for arn in arns:
            tt2 = str(arn)
        ...
        if ":" + context.acc + ":" in tt2:
            while ":" + context.acc + ":" in tt2:
                ...
                if context.debug:
                    log.debug("out tt2=" + tt2)
                if "[" in tt2:
                    tt2 = tt2.lstrip("[").rstrip("]").lstrip('"').rstrip('"')
                    if context.debug:
                        log.debug("in tt2=" + tt2)
            ...
            for i in range(cc + 1):
                ...
                if ttft == "aws_subnet":
                    try:
                        ...
                        log.warning(
                            "WARNING: subnet not in subnet list "
                            + subn
                ...
                # security_group
                    ...
                    try:
                        ...
                        log.warning(
                            "WARNING: security group not in sg list "
                            + subn
                            + " Resource may be referencing a security group that no longer exists"
                        )
                        subs = subs + '"' + subn + '"' + ","
                #
                else:
...
def deref_role_arn(t1, tt1, tt2):
    ...
    if tt2.startswith("arn:aws:events:"):
        log.debug(tt2)
    ...
    if tt2.startswith("arn:aws:s3:::"):
        ...
        tt2.startswith("arn:aws:events:")
        and ":rule/" in tt2
        and ":rule/aws.partner" not in tt2
    ):
        rn = tt2.split("/")[-1]
    ...
    try:
        ...
        if cc == 0 and ":log-stream:" in tt2:
            ...
            if logr == context.region:
                ...
                if isstar:
                    streamn = streamn.rstrip("*")
                if isstar:
                    period = "."
                    arnadr = (
                        "aws_cloudwatch_log_stream." + logn2 + "_" + streamn + ".arn"
                    )
                    log.debug(arnadr)
