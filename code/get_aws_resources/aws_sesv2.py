...
def get_aws_sesv2_email_identity_mail_from_attributes(
        ...
        if id is None:
            ...
            for j in response.get("EmailIdentities", []):
                ...
                try:
                    ...
                    if "MailFromAttributes" in detail and detail[
                        "MailFromAttributes"
                    ].get("MailFromDomain"):
                    ...
                    if common.context.debug:
                        common.log.debug(
                            f"No mail-from attributes for {identity_name}: {e}"
                        )
                    continue
        else:
            ...
            if (
                response
                and "MailFromAttributes" in response
                and response["MailFromAttributes"].get("MailFromDomain")
            ):
                common.write_import(type, id, None)
    except Exception as e:
        common.handle_error(
...
def get_aws_sesv2_email_identity_feedback_attributes(
    type, id, clfn, descfn, topkey, key, filterid
):
    try:
        config = Config(retries={"max_attempts": 10, "mode": "standard"})
        ...
        if id is None:
            ...
            for j in response.get("EmailIdentities", []):
                ...
                try:
                    ...
                    if common.context.debug:
                        common.log.debug(
                            f"No feedback attributes for {identity_name}: {e}"
                        )
                    continue
        else:
...
def get_aws_sesv2_configuration_set_event_destination(
    type, id, clfn, descfn, topkey, key, filterid
):
    try:
        config = Config(retries={"max_attempts": 10, "mode": "standard"})
        ...
        if id is None:
            ...
            for config_set_name in config_sets:
                try:
                    ...
                    if common.context.debug:
                        common.log.debug(
                            f"No event destinations for config set {config_set_name}: {e}"
                        )
                    continue
        else:
...
def get_aws_sesv2_dedicated_ip_assignment(
    type, id, clfn, descfn, topkey, key, filterid
):
    try:
        config = Config(retries={"max_attempts": 10, "mode": "standard"})
        ...
        if id is None:
            ...
            if "," in id:
                ...
                try:
                    ...
                    if (
                        response
                        and response.get("DedicatedIp", {}).get("PoolName") == pool
                    ):
                        common.write_import(type, id, None)
                except Exception as e:
                    if common.context.debug:
...
def get_aws_sesv2_account_suppression_attributes(
    type, id, clfn, descfn, topkey, key, filterid
):
    try:
        config = Config(retries={"max_attempts": 10, "mode": "standard"})
        ...
        if id is None or id == account_id:
            try:
                ...
                if common.context.debug:
                    common.log.debug(
                        f"Account suppression attributes not configured: {e}"
                    )
    except Exception as e:
        common.handle_error(
            e, str(inspect.currentframe().f_code.co_name), clfn, descfn, topkey, id
        ...
        if id is None:
            ...
            for identity in identities:
                ...
                try:
                    ...
                    policy_response = client.get_email_identity_policies(
                    ...
                    if common.context.debug:
                        common.log.debug(
                            f"No policies for identity {identity_name}: {e}"
                        )
                    continue
        else:
