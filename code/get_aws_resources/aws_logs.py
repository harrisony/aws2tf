...
def get_aws_cloudwatch_log_group(type, id, clfn, descfn, topkey, key, filterid):
    if context.debug:
        log.debug(
            "--> In get_aws_cloudwatch_log_group  doing "
            + type
            + " with id "
            + str(id)
            + " clfn="
...
def get_aws_cloudwatch_log_stream(type, id, clfn, descfn, topkey, key, filterid):
    if context.debug:
        log.debug(
            "--> In get_aws_cloudwatch_log_stream  doing "
            + type
            + " with id "
            + str(id)
            + " clfn="
...
def get_aws_cloudwatch_log_data_protection_policy(
            ...
            for page in paginator.paginate():
                for log_group in page["logGroups"]:
                    ...
                    try:
                        ...
                        if context.debug:
                            log.debug(
                                f"No data protection policy for log group {log_group_name}"
                            )
                        continue
                    except Exception as e:
                        if context.debug:
                            log.debug(
                                f"Error getting policy for log group {log_group_name}: {e}"
                            )
                        continue
        else:
...
def get_aws_cloudwatch_log_destination_policy(
    type, id, clfn, descfn, topkey, key, filterid
):
    """
            ...
            for page in paginator.paginate(DestinationNamePrefix=id):
                for destination in page[topkey]:
                    if destination[key] == id:
                        if (
                            "accessPolicy" in destination
                            and destination["accessPolicy"]
                        ):
                            common.write_import(type, id, None)
                        else:
            ...
            for page in log_group_paginator.paginate():
                for log_group in page["logGroups"]:
                    ...
                    try:
                        ...
                        for filter_page in filter_paginator.paginate(
                        ...
                        if context.debug:
                            log.debug(
                                f"Error listing metric filters for log group {log_group_name}: {e}"
                            )
                        continue
        else:
            ...
            if ":" in id:
                ...
                try:
                    ...
                    response = client.describe_metric_filters(
                        logGroupName=log_group_name, filterNamePrefix=filter_name
                    )
                ...
                log_warning(
                    f"Invalid ID format for {type}: expected log-group-name:filter-name, got {id}"
                )
...
def get_aws_cloudwatch_log_subscription_filter(
    type, id, clfn, descfn, topkey, key, filterid
):
    """
            ...
            for page in log_group_paginator.paginate():
                for log_group in page["logGroups"]:
                    ...
                    try:
                        ...
                        if context.debug:
                            log.debug(
                                f"Error listing subscription filters for log group {log_group_name}: {e}"
                            )
                        continue
        else:
            # Handle composite ID in specific import
