...
def get_aws_prometheus_query_logging_configuration(
    ...
    try:
        ...
        if id is None:
            ...
            if id.startswith("arn:"):
                ...
                if context.debug:
                    log.debug("Invalid workspace ID format: " + id)
                return True
        ...
        if id is None:
            ...
            try:
                ...
                if e.response["Error"]["Code"] in [
                    ...
                    if context.debug:
                        log.debug("No policy for workspace: " + id)
                else:
                    raise
...
def get_aws_prometheus_workspace_configuration(
    type, id, clfn, descfn, topkey, key, filterid
):
    if context.debug:
        log.debug(
