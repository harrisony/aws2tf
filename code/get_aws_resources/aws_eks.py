...
def get_aws_eks_access_policy_association(
    ...
    try:
        ...
        if "," not in id:
            log.debug(
                "Must pass cluster-name,principa_arn to get_aws_eks_access_policy_association returning"
            )
            return True
        ...
        if "aws-service-role" in parn:
            log.info(
                "INFO: aws-service-role in get_aws_eks_access_policy_association returning"
            )
            context.rproc[pkey] = True
            return True
        client = boto3.client(clfn)
        response = client.list_associated_access_policies(
            clusterName=cln, principalArn=parn
        )
