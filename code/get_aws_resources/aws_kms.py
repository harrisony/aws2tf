...
def get_aws_kms_key(type, id, clfn, descfn, topkey, key, filterid):
    ...
    try:
        for j in response:
            ...
            if id is not None and not id.startswith("k-"):
                id = "k-" + id
            if id is not None and id != ka:
                ...
                try:
                    ...
                    if kstatus == "Enabled" or kstatus == "Disabled":
                        if kman == "AWS":
                            if context.debug:
                                log.debug("key " + str(theid) + " is managed by AWS")
                            pkey = type + "." + theid
                            if not context.rproc[pkey]:
                                context.rproc[pkey] = True
                            pkey = type + "." + ka
                    ...
                    if context.debug:
                        log_warning("WARNING: can't access key %s", theid)
        ...
        for j in response:
            try:
                ...
                if aliasname.startswith("alias/aws"):
                    if context.debug:
                        log.debug("Skipping " + aliasname + " " + theid)
                    if id is not None:
                        if not id.startswith("k-"):
                            id = "k-" + id
                        pkey = type + "." + id
                        context.rproc[pkey] = True
                    continue
            ...
            if id is not None:
                if id == aliasname or "alias/" + id == aliasname:
                    if context.debug:
                        log.debug("KMS ALAIS: Alias match importing  %s", id)
            ...
            if id is not None and id != ka:
                ...
                if context.debug:
                    log.debug("KMS ALAIS: Id match importing  %s", id)
