        ...
        if id is None:
            ...
            for bucket in buckets:
                ...
                try:
                    ...
                    if context.debug:
                        log.debug(
                            f"Error listing indexes for bucket {bucket_name}: {e}"
                        )
                    continue
...
def get_aws_s3vectors_vector_bucket_policy(
    ...
    try:
        ...
        if id is None:
            ...
            for bucket in buckets:
                ...
                try:
                    policy_response = client.get_vector_bucket_policy(
                    ...
                    if context.debug:
                        log.debug(f"No policy for bucket {bucket_arn}")
                    continue
                except Exception as e:
                    if context.debug:
                        log.debug(f"Error getting policy for bucket {bucket_arn}: {e}")
                    continue
