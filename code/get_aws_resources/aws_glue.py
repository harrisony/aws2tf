...
def get_aws_glue_catalog_database(type, id, clfn, descfn, topkey, key, filterid):
    ...
    if context.debug:
        log.debug(
            "--> Inn get_aws_glue_catalog_database  doing "
            + type
            + " with id "
            + str(id)
            + " clfn="
    ...
    try:
        ...
        if (
            exn == "AccessDeniedException"
            and "Insufficient Lake Formation permission" in str(e)
        ):
            log.info(
                "AccessDeniedException - Insufficient Lake Formation permission for %s %s",
                type,
                id,
            )
            if id is not None:
...
def get_aws_glue_catalog_table(type, id, clfn, descfn, topkey, key, filterid):
    ...
    if context.debug:
        log.debug(
            "--> In get_aws_glue_catalog_table  doing "
            + type
            + " with id "
            + str(id)
            + " clfn="
    ...
    try:
        ...
        if id is None:
            log_warning(
                "WARNING: ID can not be None - must pass catalog:database or catalog:database:tablename"
            )
            return True
        cc = id.count(":")
        if cc == 0:
            log_warning(
                "WARNING: ID - must pass catalog:database or catalog:database:tablename"
            )
            return True
        if cc == 1:
            catalogn = id.split(":")[0]
        ...
        if cc == 2:
            response = client.get_tables(
                CatalogId=catalogn, DatabaseName=databasen, Expression=tabnam
            )
        ...
        if e.response["Error"]["Code"] == "AccessDeniedException":
            log.info(
                f"AccessDeniedException for aws_glue.py - returning. Resource: {id}"
            )
            tkey = "aws_glue_catalog_table" + "." + catalogn + ":" + databasen
            context.rproc[tkey] = True
            return True
...
def get_aws_glue_dev_endpoint(type, id, clfn, descfn, topkey, key, filterid):
    ...
    if context.debug:
        log.debug(
            "--> In get_aws_glue_dev_endpoint doing "
            + type
            + " with id "
            + str(id)
            + " clfn="
...
def get_aws_glue_data_catalog_encryption_settings(
    ...
    try:
        ...
        if id is None:
            ...
            if response["DataCatalogEncryptionSettings"] == []:
                log.debug(
                    "Empty response for " + type + " id=" + str(id) + " returning"
                )
                return True
...
def get_aws_glue_connection(type, id, clfn, descfn, topkey, key, filterid):
    ...
    if context.debug:
        log.debug(
            "--> In get_aws_glue_connection doing "
            + type
            + " with id "
            + str(id)
            + " clfn="
    ...
    try:
        ...
        if id is None:
            ...
            if cc == 2:
                ...
                log_warning(
                    "WARNING: Invalid aws_glue_partition id passed must pass catalogid:database:tablename got: "
                    + id
                    + "c="
                    + str(cc)
                )
            ...
            try:
                response = client.get_partitions(
                    CatalogId=catalogn, DatabaseName=databasen, TableName=tabnam
                )
            except Exception as e:
                log.info(e)
                context.rproc[tkey] = True
        ...
        if id is None:
                ...
                try:
                    ...
                    for table in tables:
                        ...
                        try:
                            ...
                            for idx in indexes:
                                composite_id = (
                                    f"{context.acc}:{db_name}:{table_name}:{idx[key]}"
                                )
                                common.write_import(type, composite_id, None)
                        except Exception as e:
                            if context.debug:
                                log.debug(
                                    f"Error getting indexes for table {table_name}: {e}"
                                )
                            continue
                except Exception as e:
                    if context.debug:
                        log.debug(f"Error getting tables for database {db_name}: {e}")
                    continue
        else:
            ...
            if id.count(":") == 3:
                ...
                try:
                    ...
                    if context.debug:
                        log.debug(f"Error getting specific index: {e}")
            else:
                if context.debug:
                    log.debug(
                        "Must pass catalog_id:database_name:table_name:index_name for "
                        + type
                    )
...
def get_aws_glue_resource_policy(type, id, clfn, descfn, topkey, key, filterid):
    ...
    try:
        ...
        try:
            ...
            if context.debug:
                log.debug(f"No resource policy found for region {region}")
        except Exception as e:
            if context.debug:
                log.debug(f"Error getting resource policy: {e}")
        ...
        if id is None:
            ...
            for db in databases:
                ...
                try:
                    ...
                    if context.debug:
                        log.debug(
                            f"Error getting functions for database {db_name}: {e}"
                        )
                    continue
        else:
            ...
            if id.count(":") == 2:
                ...
                try:
                    ...
                    if context.debug:
                        log.debug(f"Error getting specific function: {e}")
            else:
                if context.debug:
                    log.debug(
                        "Must pass catalog_id:database_name:function_name for " + type
                    )
