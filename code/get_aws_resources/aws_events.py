        ...
        if id is None:
            ...
            if id == "default":
                ...
                try:
                    ...
                    log.debug(
                        "ADVICE: Check what other resources may be referring to this resource if it doesn't exist"
                    )
                    pkey = "aws_cloudwatch_event_rule." + id
                    context.rproc[pkey] = True
                    return True
...
def get_aws_cloudwatch_event_target(type, id, clfn, descfn, topkey, key, filterid):
    ...
    try:
        ...
        if id is None:
            log_warning(
                "WARNING: Must pass event_bus_name/rule-name as a parameter returning"
            )
            return True
    ...
    try:
        ...
        if id is None:
            ...
            if "/" in id:
                ...
                log_warning(
                    f"Invalid ID format for {type}: expected event-bus-name/statement-id, got {id}"
                )
