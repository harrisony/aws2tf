    ...
    try:
        ...
        if id is None:  # assume scope = cloudfront
            ...
            if context.region == "us-east-1":
                ...
                log_warning(
                    "WARNING:Can only import CLOUDFRONT ip sets from us-east-1 region"
                )
            log.debug("INFO: Getting Regional resources")
            sc = "REGIONAL"
            response = client.list_ip_sets(Scope=sc)
            if response[topkey] == []:
                if context.debug:
    ...
    try:
        ...
        if id is None:  # assume scope = cloudfront
            ...
            if context.region == "us-east-1":
                ...
                log_warning(
                    "WARNING:Can only import CLOUDFRONT web ACL's from us-east-1 region"
                )
...
# aws_wafv2_web_acl_logging_configuration ARN
def get_aws_wafv2_web_acl_logging_configuration(
            ...
            if id.startswith("arn:"):
                ...
                for rtype in rtypes:
                    response = client.list_resources_for_web_acl(
                        WebACLArn=id, ResourceType=rtype
                    )
