        ...
        if id is None:
            ...
            for page in paginator.paginate():
                try:
                    ...
                    log.debug(
                        "No DistributionList in response for "
                        + type
                        + " id="
                        + str(id)
                        + " returning"
...
def get_aws_cloudfront_origin_access_identity(
...
def get_aws_cloudfront_continuous_deployment_policy(
    type, id, clfn, descfn, topkey, key, filterid
):
    if context.debug:
        log.debug(
...
def get_aws_cloudfront_field_level_encryption_config(
    type, id, clfn, descfn, topkey, key, filterid
):
    if context.debug:
        log.debug(
...
def get_aws_cloudfront_field_level_encryption_profile(
    type, id, clfn, descfn, topkey, key, filterid
):
    if context.debug:
        log.debug(
...
def get_aws_cloudfront_response_headers_policy(
    type, id, clfn, descfn, topkey, key, filterid
):
    if context.debug:
        log.debug(
...
def get_aws_cloudfront_origin_access_control(
    type, id, clfn, descfn, topkey, key, filterid
):
    if context.debug:
        log.debug(
...
def get_aws_cloudfront_origin_request_policy(
    type, id, clfn, descfn, topkey, key, filterid
):
    if context.debug:
        log.debug(
...
def get_aws_cloudfront_realtime_log_config(
    type, id, clfn, descfn, topkey, key, filterid
):
    if context.debug:
        log.debug(
