    ...
    try:
        ...
        if id is None:
            ...
            for j in response:
                ...
                common.write_import(
                    "aws_networkfirewall_logging_configuration", j[key], None
                )
...
def get_aws_networkfirewall_firewall_policy(
        ...
        if id is None:
            ...
            if id.startswith("arn:"):
                if ":stateless-rulegroup/" in id:
                    response = client.describe_rule_group(
                        RuleGroupArn=id, AnalyzeRuleGroup=False, Type="STATELESS"
                    )
            else:
            ...
            if id.startswith("arn:"):
                if ":stateful-rulegroup/" in id:
                    response = client.describe_rule_group(
                        RuleGroupArn=id, AnalyzeRuleGroup=False, Type="STATEFUL"
                    )
            else:
                log_warning("INFO: must pass arn for rule group")
            if response == []:
...
def get_aws_networkfirewall_tls_inspection_configuration(
    type, id, clfn, descfn, topkey, key, filterid
):
    if context.debug:
        log.debug(
    ...
    try:
        ...
        if id is None:
            ...
            if id.startswith("arn:"):
                response = client.describe_tls_inspection_configuration(
                    TlsInspectionConfigurationArn=id
                )
            else:
                response = client.describe_tls_inspection_configuration(
