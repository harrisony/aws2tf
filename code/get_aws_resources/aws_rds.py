...
def get_aws_db_parameter_group(type, id, clfn, descfn, topkey, key, filterid):
    ...
    if context.debug:
        log.debug(
            "--> In get_aws_db_parameter_group  doing "
            + type
            + " with id "
            + str(id)
            + " clfn="
...
def get_aws_db_option_group(type, id, clfn, descfn, topkey, key, filterid):
    ...
    if context.debug:
        log.debug(
            "--> In get_aws_db_option_group  doing "
            + type
            + " with id "
            + str(id)
            + " clfn="
...
def get_aws_db_instance(type, id, clfn, descfn, topkey, key, filterid):
    ...
    try:
        ...
        if id is None:
            ...
            for j in response:
                engine = j["Engine"]
                if engine == "docdb" or engine.startswith("aurora"):
                    continue
                common.write_import(type, j[key], None)
        ...
        log_warning(
            "WARNING: InvalidParameterValue for " + type + " " + str(id) + " returning"
        )
