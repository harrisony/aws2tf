...
def get_aws_bedrockagentcore_agent_runtime(
...
def get_aws_bedrockagentcore_workload_identity(
    type, id, clfn, descfn, topkey, key, filterid
):
    try:
        config = Config(retries={"max_attempts": 10, "mode": "standard"})
...
def get_aws_bedrockagentcore_code_interpreter(
    type, id, clfn, descfn, topkey, key, filterid
):
    try:
        config = Config(retries={"max_attempts": 10, "mode": "standard"})
...
def get_aws_bedrockagentcore_token_vault_cmk(
    type, id, clfn, descfn, topkey, key, filterid
):
    """
