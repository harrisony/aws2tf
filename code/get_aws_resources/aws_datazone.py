            ...
            for ut in ["ASSET_TYPE", "FORM_TYPE", "LINEAGE_NODE_TYPE"]:
                ...
                try:
                    for page in paginator.paginate(
                        domainIdentifier=id, searchScope=ut, managed=False
                    ):
        ...
        if id is None:
            ...
            for ut in [
                ...
                try:
                    ...
                    if "ResourceNotFoundException" in str(e):
                        if context.debug:
                            log.debug("Resource not found for " + ut)
                        continue
                    else:
                        log.info("ERROR: " + str(e))
                        exit()
...
def dz_common(resp2, dzid, type, client):
    ...
    dst = resp2["status"]
    if context.debug:
        log.debug("DataZone sso=" + str(sso) + " dst=" + str(dst), " dzid=" + dzid)
    ...
    for k in resp3["items"]:
        ...
        pn = k["provider"]
        if context.debug:
            log.debug(str(bid), str(bn), str(pn))
        output = StringIO()
        output.write(
        ...
        output.write(
            "  domain_id = aws_datazone_domain." + str(dzid) + ".id\n"
        )  ## error undefined
...
def get_aws_datazone_environment_blueprint_configuration(
    ...
    try:
        ...
        if id is None:
            log_warning(
                "WARNING must pass domain id to get_aws_datazone_environment_blueprint_configuration"
            )
            return True
        else:
            for page in paginator.paginate(domainIdentifier=id):
...
def get_aws_datazone_environment_profile(type, id, clfn, descfn, topkey, key, filterid):
    ...
    try:
        ...
        if id is None:
            log_warning(
                "WARNING must pass domain id to get_aws_datazone_environment_profile"
            )
            return True
        else:
            try:
                ...
                log.debug(str(e))
                log_warning(
                    "WARNING: no environment profiles found for domain id " + id
                )
...
def get_aws_datazone_environment(type, id, clfn, descfn, topkey, key, filterid):
    ...
    try:
        ...
        if id is None:
            log_warning(
                "WARNING must pass domain_id:project_id to get_aws_datazone_environment"
            )
            return True
        else:
            dzd = id.split(":")[0]
            pid = id.split(":")[1]
            for page in paginator.paginate(
                domainIdentifier=dzd, projectIdentifier=pid, status="ACTIVE"
            ):
                response = response + page[topkey]
        pkey = type + "." + id
        if response == []:
