...
def get_aws_devopsguru_event_sources_config(
    ...
    try:
        ...
        if id is None:
            ...
            try:
                ...
                if context.debug:
                    print(
                        f"DevOps Guru event sources config not found in region {client.meta.region_name}"
                    )
...
def get_aws_devopsguru_notification_channel(
    type, id, clfn, descfn, topkey, key, filterid
):
    try:
        config = Config(retries={"max_attempts": 10, "mode": "standard"})
...
def get_aws_devopsguru_resource_collection(
    type, id, clfn, descfn, topkey, key, filterid
):
    """
            ...
            for collection_type in collection_types:
                try:
                    response = client.get_resource_collection(
                        ResourceCollectionType=collection_type
                    )
...
def get_aws_devopsguru_service_integration(
    type, id, clfn, descfn, topkey, key, filterid
):
    """
    Regional singleton resource - uses region as ID
    ...
    try:
        ...
        if id is None:
            ...
            try:
                ...
                if context.debug:
                    print(
                        f"DevOps Guru service integration not found in region {client.meta.region_name}"
                    )
            except Exception as e:
                if context.debug:
