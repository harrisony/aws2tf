...
def get_aws_ecs_task_definition(type, id, clfn, descfn, topkey, key, filterid):
    ...
    if context.debug:
        log.debug(
            "--> get_aws_ecs_task_definition  doing "
            + type
            + " with id "
            + str(id)
            + " clfn="
...
def get_aws_ecs_capacity_provider(type, id, clfn, descfn, topkey, key, filterid):
    ...
    if context.debug:
        log.debug(
            "--> get_aws_ecs_capacity_provider  doing "
            + type
            + " with id "
            + str(id)
            + " clfn="
...
def get_aws_ecs_cluster_capacity_providers(
    type, id, clfn, descfn, topkey, key, filterid
):

    if context.debug:
        log.debug(
            "--> get_aws_ecs_cluster_capacity_provider  doing "
            + type
            + " with id "
            + str(id)
            + " clfn="
