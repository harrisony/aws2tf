...
def get_aws_organizations_organizational_unit(
        ...
        if scpskip:
            ...
            for j in response[topkey]:
                if "AWS" not in j[key]:
                    common.write_import(type, j[key], None)
                    common.add_known_dependancy(
                        "aws_organizations_policy_attachment", j[key]
                    )
...
def get_aws_organizations_resource_policy(
    type, id, clfn, descfn, topkey, key, filterid
):
    if context.debug:
        log.debug(
    ...
    try:
        ...
        try:
            ...
            if exn == "ResourcePolicyNotFoundException":
                ...
                log.info(
                    "Can't describe_resource_policy - no access or not a master account......"
                )
                return True
            # else:
            #    print(f"{e=} [org1]")
...
def get_aws_organizations_policy_attachment(
    type, id, clfn, descfn, topkey, key, filterid
):
    if context.debug:
        log.debug(
