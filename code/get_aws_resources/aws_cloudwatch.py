    ...
    try:
        ...
        if id is None:
            ...
            response = client.describe_alarms(
                AlarmNames=[id], AlarmTypes=["MetricAlarm"]
            )
            if response[topkey]:
                j = response[topkey][0]
                common.write_import(type, j[key], None)
...
def get_aws_cloudwatch_composite_alarm(type, id, clfn, descfn, topkey, key, filterid):
    ...
    try:
        ...
        if id is None:
            ...
            response = client.describe_alarms(
                AlarmNames=[id], AlarmTypes=["CompositeAlarm"]
            )
            if response[topkey]:
                j = response[topkey][0]
                common.write_import(type, j[key], None)
...
def get_aws_cloudwatch_contributor_insight_rule(
...
def get_aws_cloudwatch_contributor_managed_insight_rule(
    type, id, clfn, descfn, topkey, key, filterid
):
    """
