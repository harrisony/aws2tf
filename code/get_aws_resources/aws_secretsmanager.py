...
def get_aws_secretsmanager_secret(type, id, clfn, descfn, topkey, key, filterid):
    if context.debug:
        log.debug(
            "--> In get_aws_secretsmanager_secret  doing "
            + type
            + " with id "
            + str(id)
            + " clfn="
                ...
                if sn.startswith("rds!"):
                    ...
                    try:
                        ...
                        common.add_dependancy(
                            "aws_secretsmanager_secret_rotation", j[key]
                        )
                    except KeyError:
...
def get_aws_secretsmanager_secret_rotation(
    type, id, clfn, descfn, topkey, key, filterid
):
    if context.debug:
        log.debug(
            "--> In get_aws_secretsmanager_secret_rotation  doing "
            + type
            + " with id "
            + str(id)
            + " clfn="
    ...
    try:
        ...
        if id is None:
            log.debug(
                "ERROR: get_aws_secretsmanager_secret_rotation must be called with SecretID as parameter"
            )
        else:
            pkey = type + "." + id
            response = client.describe_secret(SecretId=id)
...
def get_aws_secretsmanager_secret_version(
    type, id, clfn, descfn, topkey, key, filterid
):
    if context.debug:
        log.debug(
            "--> In get_aws_secretsmanager_secret_version  doing "
            + type
            + " with id "
            + str(id)
            + " clfn="
    ...
    try:
        ...
        if id is None:
            ...
            log.debug(
                "ERROR: get_aws_secretsmanager_secret_verion must be called with SecretID as parameter"
            )
        else:
            response = client.list_secret_version_ids(
                SecretId=id, IncludeDeprecated=False
            )
            if response == []:
                log.debug(
                    "Empty response for " + type + " id=" + str(id) + " returning"
            ...
            for j in response[topkey]:
                try:
                    ...
                    if "(AccessDeniedException) when calling the GetSecretValue" in str(
                        e
                    ):
                        log.debug(
                            "INFO: get_secret_value failed - not authorized skipping %s %s",
                            type,
                            id.split(":")[-1],
                        )
                        pkey = type + "." + id
...
def get_aws_secretsmanager_secret_policy(type, id, clfn, descfn, topkey, key, filterid):
    if context.debug:
        log.debug(
            "--> In get_aws_secretsmanager_secret_policy  doing "
            + type
            + " with id "
            + str(id)
            + " clfn="
    ...
    try:
        response = []
        log.debug(
            "INFO: aws_secretsmanager_policy - policy embedded in aws_secretmager_secret by aws2tf"
        )
