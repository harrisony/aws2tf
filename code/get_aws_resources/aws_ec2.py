...
def get_aws_vpc(type, id, clfn, descfn, topkey, key, filterid):
    ...
    try:
        if vpc_id is None:
            ...
            try:
                if context.vpclist[vpc_id]:
                    ...
                    if not context.dnet:
                        common.add_known_dependancy(
                            "aws_route_table_association", vpc_id
                        )
                        common.add_dependancy("aws_route_table_association", vpc_id)
                        common.add_dependancy(
                            "aws_vpc_ipv4_cidr_block_association", vpc_id
                        )
                ...
                log_warning(
                    "WARNING: vpc not in vpclist "
                    + vpc_id
                    + " Resource may be referencing a vpc that no longer exists"
                )
        ...
        common.handle_error(
            e, str(inspect.currentframe().f_code.co_name), clfn, descfn, topkey, vpc_id
        )
    ...
    try:
        if id is None:
            ...
            try:
                ...
                log_warning(
                    "WARNING: subnet not in subnetlist "
                    + id
                    + " Resource may be referencing a subnet that no longer exists"
                )
...
def get_aws_security_group(type, id, clfn, descfn, topkey, key, filterid):
    if context.debug:
        log.debug(
            "--> In get_aws_security_group doing "
            + type
            + " with id "
            + str(id)
            + " clfn="
    ...
    try:
        if id is None:
            ...
            try:
                ...
                log_warning(
                    "WARNING: sg not in sglist "
                    + id
                    + " Resource may be referencing a security_group that no longer exists"
                )

        else:
            log_warning(
                "WARNING: get_aws_security_group unexpected id value %s", tr(id)
            )
            return True
...
def get_aws_instance(type, id, clfn, descfn, topkey, key, filterid):
    ...
    try:
        ...
        session = boto3.Session(
            region_name=context.region, profile_name=context.profile
        )
            ...
            for j in response:
                for k in j["Instances"]:
                    if context.ec2tag is None:
                        if (
                            k["State"]["Name"] == "running"
                            or k["State"]["Name"] == "stopped"
                        ):
...
def get_aws_security_group_rule(type, id, clfn, descfn, topkey, key, filterid):
    ...
    if context.debug:
        log.debug(
            "--> In get_security_group_rule doing "
            + type
            + " with id "
            + str(id)
            + " clfn="
    ...
    try:
        ...
        for j in response[topkey]:
            ...
            if protocol != "":
                impstring = impstring + "_" + protocol
            if fromport != "":
                impstring = impstring + "_" + fromport
            if toport != "":
                impstring = impstring + "_" + toport
            if cidr4 != "":
                impstring = impstring + "_" + cidr4
            if cidr6 != "":
                impstring = impstring + "_" + cidr6
            if refgrp_sgid != "":
                impstring = impstring + "_" + refgrp_sgid
            # else:
            #    if cidr4 != "": impstring= impstring + "_" + cidr4
            #    if cidr6 != "": impstring= impstring + "_" + cidr6
            if prefixlistid != "":
                impstring = impstring + "_" + prefixlistid
...
def get_aws_eip_association(type, id, clfn, descfn, topkey, key, filterid):
    ...
    if context.debug:
        log.debug(
            "--> In get_aws_eip_assocation doing "
            + type
            + " with id "
            + str(id)
            + " clfn="
...
def get_aws_route_table_association(type, id, clfn, descfn, topkey, key, filterid):
    ...
    try:
        if context.debug:
            log.debug(
                "--> In get_aws_route_table_association doing "
                + type
                + " with id "
                + str(id)
                + " clfn="
        ...
        if id is not None:
            if "subnet-" in id:
                for page in paginator.paginate(
                    Filters=[
                        {"Name": "association.subnet-id", "Values": [id]},
                    ]
                ):
...
def get_aws_launch_template(type, id, clfn, descfn, topkey, key, filterid):
    if context.debug:
        log.debug(
            "--> In get_aws_launch_template    doing "
            + type
            + " with id "
            + str(id)
            + " clfn="
...
def get_aws_vpc_ipv4_cidr_block_association(
    type, id, clfn, descfn, topkey, key, filterid
):
    if context.debug:
        log.debug(
            "--> In get_aws_vpc_ipv4_cidr_block_association doing "
            + type
            + " with id "
            + str(id)
            + " clfn="
    ...
    try:
        if id is not None:
            if "acl-" in id:
                for page in paginator.paginate(
                    Filters=[
                        {"Name": "association.network-acl-id", "Values": [id]},
...
def get_aws_default_network_acl(type, id, clfn, descfn, topkey, key, filterid):
    if context.debug:
        log.debug(
            "--> In get_aws_default_network_acl doing "
            + type
            + " with id "
            + str(id)
            + " clfn="
...
def get_aws_vpc_endpoint_route_table_association(
    type, id, clfn, descfn, topkey, key, filterid
):
    if context.debug:
        log.debug(
    ...
    try:
        ...
        if id is None:
            ...
            for j in response:
                common.write_import(type, j[key], None)
                common.add_known_dependancy(
                    "aws_ec2_transit_gateway_vpc_attachment", j[key]
                )
                common.add_known_dependancy(
                    "aws_ec2_transit_gateway_peering_attachment", j[key]
                )
                common.add_known_dependancy(
                    "aws_ec2_transit_gateway_route_table", j[key]
                )
                common.add_known_dependancy(
                    "aws_ec2_transit_gateway_vpn_attachment", j[key]
                )
...
def get_aws_ec2_transit_gateway_vpc_attachment(
    type, id, clfn, descfn, topkey, key, filterid
):
    if context.debug:
        log.debug(
        ...
        if id is None:
            ...
            if id.startswith("tgw-"):
                response = client.describe_transit_gateway_vpc_attachments(
                    Filters=[{"Name": "transit-gateway-id", "Values": [id]}]
                )
                if response[topkey] == []:
                    log.debug(
                        "Empty response for " + type + " id=" + str(id) + " returning"
...
def get_aws_ec2_transit_gateway_peering_attachment(
    type, id, clfn, descfn, topkey, key, filterid
):
    if context.debug:
        log.debug(
...
def get_aws_ec2_transit_gateway_route_table(
    type, id, clfn, descfn, topkey, key, filterid
):
    if context.debug:
        log.debug(
...
def get_aws_ec2_transit_gateway_route(type, id, clfn, descfn, topkey, key, filterid):
    ...
    try:
        ...
        if id is None:
            log_warning(
                "WARNING: " + type + " id must pass Transit gateway route table id"
            )
...
def get_aws_ec2_transit_gateway_vpn_attachment(
    type, id, clfn, descfn, topkey, key, filterid
):
    if context.debug:
        log.debug(
        ...
        if id is None:
            ...
            for page in paginator.paginate(
                Filters=[
                    ...
                    {
                        "Name": "state",
                        "Values": ["available", "pending", "pendingAcceptance"],
                    },
                ]
            ):
                response = response + page[topkey]
            ...
            for j in response:
                ...
                with open(fn, "w") as f:
                    ...
                    f.write(
                        " vpn_connection_id  = aws_vpn_connection." + vpnid + ".id \n"
                    )
                    f.write("}\n")
            ...
            if id.startswith("tgw-"):
                ...
                for j in response[topkey]:
                    ...
                    with open(fn, "w") as f:
                        ...
                        f.write(
                            " vpn_connection_id  = aws_vpn_connection."
                            + vpnid
                            + ".id \n"
                        )
                        f.write("}\n")
    ...
    try:
        ...
        if id is None:
            log_warning(
                "WARNING: "
                + type
                + " id is None - must pass eni-xxxxxxxxxx as paramter returning"
            )
            return True
        ...
        if id is None:
            ...
            for j in response:
                ...
                for kk in k:
                    ...
                    common.add_dependancy(
                        "aws_verifiedaccess_instance_trust_provider_attachment", pkey
                    )

        elif id.startswith("vai-"):
            response = client.describe_verified_access_instances(
                Filters=[
...
def get_aws_verifiedaccess_instance_trust_provider_attachment(
    type, id, clfn, descfn, topkey, key, filterid
):
    # if context.debug:
    log.debug(
    ...
    try:
        ...
        if id is None:
            ...
            response = client.describe_verified_access_trust_providers(
                VerifiedAccessTrustProviderIds=[pid]
            )
            if response == []:
                if context.debug:
                    log.debug(
...
def get_aws_verifiedaccess_endpoint(type, id, clfn, descfn, topkey, key, filterid):
    if context.debug:
        log.debug(
            "--> In "
            + str(inspect.currentframe().f_code.co_name)
    ...
    try:
        ...
        elif id.startswith("vae-"):
            response = client.describe_verified_access_endpoints(
                VerifiedAccessEndpointIds=[id]
            )
            if response == []:
                if context.debug:
                    log.debug(
...
def get_aws_vpc_block_public_access_exclusion(
    type, id, clfn, descfn, topkey, key, filterid
):
    try:
...
def get_aws_vpc_block_public_access_options(
    type, id, clfn, descfn, topkey, key, filterid
):
    try:
        from botocore.config import Config
...
def get_aws_ec2_default_credit_specification(
    type, id, clfn, descfn, topkey, key, filterid
):
    try:
        from botocore.config import Config
        ...
        if id is None:
            ...
            for page in paginator.paginate():
                for subnet in page["Subnets"]:
                    ...
                    try:
                        reservations = client.get_subnet_cidr_reservations(
                            SubnetId=subnet_id
                        )
                        for reservation in reservations.get(
                        ...
                        if context.debug:
                            log.debug(f"No reservations for subnet {subnet_id}: {e}")
                        continue
        else:
            ...
            if ":" in id:
                ...
                try:
                    ...
                    if context.debug:
                        log.debug(f"Error getting reservation {id}: {e}")
            elif id.startswith("subnet-"):
                ...
                try:
                    ...
                    if context.debug:
                        log.debug(f"Error getting reservations for subnet {id}: {e}")
    except Exception as e:
        common.handle_error(
            e, str(inspect.currentframe().f_code.co_name), clfn, descfn, topkey, id
        )
