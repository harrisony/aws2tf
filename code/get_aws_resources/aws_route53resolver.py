...
def get_aws_route53_resolver_dnssec_config(
...
def get_aws_route53_resolver_firewall_rule(
    type, id, clfn, descfn, topkey, key, filterid
):
    try:
        from botocore.config import Config
        ...
        if id is None:
            ...
            for rule_group in rule_groups:
                ...
                try:
                    ...
                    for rules_page in rules_paginator.paginate(
                        ...
                        for rule in rules_page["FirewallRules"]:
                            ...
                            if "FirewallDomainListId" in rule:
                                composite_id = (
                                    f"{rule_group_id}:{rule['FirewallDomainListId']}"
                                )
                    ...
                    if context.debug:
                        log.debug(f"Error listing rules for group {rule_group_id}: {e}")
                    continue
        else:
            ...
            if ":" in id:
                ...
                try:
                    ...
                    for rules_page in rules_paginator.paginate(
                        ...
                        for rule in rules_page["FirewallRules"]:
                            if (
                                "FirewallDomainListId" in rule
                                and rule["FirewallDomainListId"] == resource_id
                            ) or (
                                "FirewallThreatProtectionId" in rule
                                and rule["FirewallThreatProtectionId"] == resource_id
                            ):
                                common.write_import(type, id, None)
                                break
                except Exception as e:
                    if context.debug:
                        log.debug(f"Error getting rule {id}: {e}")
                    ...
                    for rules_page in rules_paginator.paginate(FirewallRuleGroupId=id):
                        for rule in rules_page["FirewallRules"]:
                            if "FirewallDomainListId" in rule:
                                ...
                                composite_id = (
                                    f"{id}:{rule['FirewallThreatProtectionId']}"
                                )
                                common.write_import(type, composite_id, None)
                except Exception as e:
                    if context.debug:
                        log.debug(f"Error listing rules for group {id}: {e}")
    except Exception as e:
        common.handle_error(
            e, str(inspect.currentframe().f_code.co_name), clfn, descfn, topkey, id
        )
