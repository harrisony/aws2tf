        ...
        if id is None:
            ...
            if id.startswith("arn:"):
                ...
                try:
                    ...
                    log.info(
                        "ERROR: This will cause problems later on and should be addressed\n"
                    )
        ...
        if id is None:
            for fn in context.lambdalist.keys():
                ...
                common.add_known_dependancy(
                    "aws_lambda_function_event_invoke_config", fn
                )
            ...
            try:
                ...
                log_warning(
                    "WARNING: function not in lambda list "
                    + id
                    + " Resource may be referencing a lambda that no longer exists"
                )
                pkey = type + "." + id
                context.rproc[pkey] = True
...
def get_aws_lambda_permission(type, id, clfn, descfn, topkey, key, filterid):
    if context.debug:
        log.debug(
            "--> In get_aws_lambda_permission doing "
            + type
            + " with id "
            + str(id)
            + " clfn="
    ...
    try:
        ...
        try:
            ...
            if context.debug:
                log_warning(
                    "WARNING: ResourceNotFoundException for "
                    + type
                    + " "
                    + str(id)
                    + " returning"
...
def get_aws_lambda_function_event_invoke_config(
    type, id, clfn, descfn, topkey, key, filterid
):
    if context.debug:
        log.debug(
            "--> In get_aws_lambda_function_event_invoke_config doing "
            + type
            + " with id "
            + str(id)
            + " clfn="
...
def get_aws_lambda_event_source_mapping(type, id, clfn, descfn, topkey, key, filterid):
    if context.debug:
        log.debug(
            "--> In get_aws_lambda_event_source_mapping doing "
            + type
            + " with id "
            + str(id)
            + " clfn="
    ...
    try:
        ...
        if response == []:
            if context.debug:
                log.debug("Empty response for " + type + " " + str(id) + " returning")
            return True
...
def get_aws_lambda_layer_version_permission(
    type, id, clfn, descfn, topkey, key, filterid
):
    if context.debug:
        log.debug(
            "--> In get_aws_lambda_layer_version_permission 1 doing "
            + type
            + " with id "
            + str(id)
            + " clfn="
...
def get_aws_lambda_function_recursion_config(
    type, id, clfn, descfn, topkey, key, filterid
):
    if context.debug:
        log.debug(
...
def get_aws_lambda_function_url(type, id, clfn, descfn, topkey, key, filterid):
    if context.debug:
        log.debug(
            "--> In get_aws_lambda_function_url doing "
            + type
            + " with id "
            + str(id)
            + " clfn="
        ...
        if id is None:
            ...
            for func in functions:
                ...
                try:
                    ...
                    if context.debug:
                        log.debug(f"No URL config for {function_name}: {e}")
                    continue
        else:
