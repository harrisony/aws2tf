    ...
    try:
        ...
        if id is None:
            ...
            try:
                ...
                common.log_warning(
                    "No queues found " + type + " id=" + str(id) + " returning"
                )
                return True
            if response == []:
                log.debug(
            ...
            if "://" not in id:
                ...
                try:
                    if context.debug:
                        log.debug("Getting URL for queue %s", id)
    ...
    try:
        ...
        if id is None:
            ...
            response = client.get_queue_attributes(
                QueueUrl=id, AttributeNames=["Policy"]
            )
            try:
                tempr = response[topkey]
            except KeyError as e:
                log.info(
                    "No policy found for " + type + " id=" + str(id) + " returning"
                )
...
def get_aws_sqs_queue_redrive_allow_policy(
    ...
    try:
        ...
        if id is None:
            ...
            response = client.get_queue_attributes(
                QueueUrl=id, AttributeNames=["RedriveAllowPolicy"]
            )
            ...
            try:
                ...
                log.debug(
                    "No redrive allow policy found for "
                    + type
                    + " id="
                    + str(id)
                    + " returning"
...
def get_aws_sqs_queue_redrive_policy(type, id, clfn, descfn, topkey, key, filterid):
    ...
    try:
        ...
        if id is None:
            ...
            response = client.get_queue_attributes(
                QueueUrl=id, AttributeNames=["RedrivePolicy"]
            )
            try:
                tempr = response[topkey]
            except KeyError as e:
                log.debug(
                    "No redrive policy found for "
                    + type
                    + " id="
                    + str(id)
                    + " returning"
