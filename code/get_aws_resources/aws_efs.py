            ...
            if id.startswith("fs-"):
                ...
                for j in response[topkey]:
                    ...
                    common.add_known_dependancy(
                        "aws_efs_replication_configuration", j[key]
                    )
                    # common.add_known_dependancy("aws_efs_lifecycle_configuration", j[key])
...
def get_aws_efs_mount_target(type, id, clfn, descfn, topkey, key, filterid):
    ...
    try:
        ...
        client = boto3.client(clfn)
        if id is None:
            log_warning("WARNING: Must pass parameter for get_aws_efs_mount_target")
            return True
        else:
            if id.startswith("fs-"):
...
def get_aws_efs_replication_configuration(
    ...
    try:
        ...
        if id is None:
            log_warning(
                "WARNING: Must pass parameter for get_aws_efs_replication_configuration"
            )
            return True
        else:
            if id.startswith("fs-"):
...
def get_aws_efs_file_system_policy(type, id, clfn, descfn, topkey, key, filterid):
    ...
    try:
        ...
        if id is None:
            log_warning(
                "WARNING: Must pass parameter for get_aws_efs_file_system_policy"
            )
            return True
        else:
            if id.startswith("fs-"):
