...
def get_aws_iam_role_policy(type, id, clfn, descfn, topkey, key, filterid):
    log.info(
        "--> In get_aws_iam_role_policy doing "
        + type
        + " with id "
        + str(id)
        + " clfn="
...
def get_aws_iam_instance_profile(type, id, clfn, descfn, topkey, key, filterid):
    if context.debug:
        log.debug(
            "--> In get_aws_iam_instance_profile  doing "
            + type
            + " with id "
            + str(id)
            + " clfn="
...
def get_aws_iam_user_group_membership(type, id, clfn, descfn, topkey, key, filterid):
    if context.debug:
        log.debug(
            "--> In get_aws_iam_user_group_membership  doing "
            + type
            + " with id "
            + str(id)
            + " clfn="
...
def get_aws_iam_user_policy(type, id, clfn, descfn, topkey, key, filterid):
    if context.debug:
        log.debug(
            "--> In get_aws_iam_user_policy  doing "
            + type
            + " with id "
            + str(id)
            + " clfn="
...
def get_aws_iam_group_policy(type, id, clfn, descfn, topkey, key, filterid):
    if context.debug:
        log.debug(
            "--> In get_aws_iam_group_policy  doing "
            + type
            + " with id "
            + str(id)
            + " clfn="
...
def get_aws_iam_role_policy_attachment(type, id, clfn, descfn, topkey, key, filterid):
    if context.debug:
        log.debug(
            "--> In get_aws_iam_role_policy_attachment doing "
            + type
            + " with id "
            + str(id)
            + " clfn="
...
def get_aws_iam_policy_attchment(type, id, clfn, descfn, topkey, key, filterid):
    if context.debug:
        log.debug(
            "--> In get_aws_iam_policy_attachment doing "
            + type
            + " with id "
            + str(id)
            + " clfn="
...
def get_aws_iam_group_policy_attachment(type, id, clfn, descfn, topkey, key, filterid):
    if context.debug:
        log.debug(
            "--> In get_aws_iam_group_policy_attachment doing "
            + type
            + " with id "
            + str(id)
            + " clfn="
            ...
            for j in response:
                ...
                for k in gresponse["Groups"]:
                    common.add_dependancy(
                        "aws_iam_user_group_membership", k["GroupName"]
                    )
...
def get_aws_iam_user_policy_attachment(type, id, clfn, descfn, topkey, key, filterid):
    if context.debug:
        log.debug(
            "--> In get_aws_iam_user_policy_attachment doing "
            + type
            + " with id "
            + str(id)
            + " clfn="
