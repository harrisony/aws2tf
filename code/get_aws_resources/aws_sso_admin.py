...
def get_aws_ssoadmin_permission_set(type, id, clfn, descfn, topkey, key, filterid):
    ...
    try:
        ...
        if id == "":
            log.debug("No SSO instance found")
            return True
        if id == context.ssoinstance:
            id = context.ssoinstance
        if context.ssoinstance is not None:
            id = context.ssoinstance
        response = []
...
def get_aws_ssoadmin_managed_policy_attachment(
    ...
    try:
        ...
        response = client.list_managed_policies_in_permission_set(
            InstanceArn=inid, PermissionSetArn=psarn
        )
        if response == []:
            if context.debug:
                log.debug(
...
def get_aws_ssoadmin_permission_set_inline_policy(
    type, id, clfn, descfn, topkey, key, filterid
):
    if context.debug:
        log.debug(
    ...
    try:
        ...
        if j == {}:
            log.debug(
                "Empty inline policy [1] for " + type + " id=" + str(id) + " returning"
            )
            rkey = type + "." + psarn + "," + inid
            context.rproc[rkey] = True
            return True
        if len(j) < 4:
            log.debug(
                "Empty inline policy [2] for " + type + " id=" + str(id) + " returning"
            )
            rkey = type + "." + psarn + "," + inid
            context.rproc[rkey] = True
            return True
