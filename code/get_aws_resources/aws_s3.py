...
def get_aws_s3_bucket_server_side_encryption_configuration(
...
def get_all_s3_buckets(fb, my_region):
    ...
    if not context.debug:
        ...
        for bucket_name in tqdm(
            accessible_buckets, desc="Processing S3 buckets", unit="bucket", leave=False
        ):
            if "aws_s3_bucket." + bucket_name in str(context.rproc):
                if context.rproc["aws_s3_bucket." + bucket_name] is True:
                    if context.debug:
                        log.debug(
                            "Already processed skipping bucket " + bucket_name + " (MT)"
                        )
                    continue
        ...
        context.tracking_message = (
            "Stage 3 of 10 getting s3 bucket properties resources ..."
        )
        log.debug(
            f"Getting S3 bucket properties for {len(accessible_buckets)} buckets..."
        )
...
def get_s3(s3_fields, type, bucket_name):
    try:
        ...
        if type == "aws_s3_bucket_replication_configuration":
            try:
                barn = str(
                    response["ReplicationConfiguration"]["Rules"][0]["Destination"][
                        "Bucket"
                    ]
                )
                try:
        ...
        if context.debug:
            log.debug("No " + type + " config for bucket " + bucket_name)
        pass


def get_s3_access_control(type, id, clfn, descfn, topkey, key, filterid):
    ret = aws_s3control.get_aws_s3_access_point(
        type, id, clfn, descfn, topkey, key, filterid
    )
    return ret
