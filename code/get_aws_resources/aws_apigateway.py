...
def get_aws_api_gateway_client_certificate(
...
def get_aws_api_gateway_documentation_part(
    type, id, clfn, descfn, topkey, key, filterid
):
...
def get_aws_api_gateway_request_validator(
    type, id, clfn, descfn, topkey, key, filterid
):
        ...
        if id is not None and "/" in id:
            ...
            try:
                response = client.get_method(
                    restApiId=restid, resourceId=resid, httpMethod="GET"
                )
                if response == []:
                    if context.debug:
                        log.debug("Empty GET response for " + type + " id=" + str(id))
                    pkey = type + "." + id
                    context.rproc[pkey] = True
                    return True
                j = response
            ...
            try:
                response = client.get_method(
                    restApiId=restid, resourceId=resid, httpMethod="POST"
                )
                if response == []:
                    if context.debug:
                        log.debug("Empty POST response for " + type + " id=" + str(id))
                    pkey = type + "." + id
                    context.rproc[pkey] = True
                    return True
                j = response
            ...
            try:
                response = client.get_method(
                    restApiId=restid, resourceId=resid, httpMethod="PUT"
                )
                if response == []:
                    if context.debug:
                        log.debug("Empty PUT response for " + type + " id=" + str(id))
                    pkey = type + "." + id
                    context.rproc[pkey] = True
                    return True
                j = response
            ...
            try:
                response = client.get_method(
                    restApiId=restid, resourceId=resid, httpMethod="DELETE"
                )
                if response == []:
                    if context.debug:
                        log.debug(
                            "Empty DELETE response for " + type + " id=" + str(id)
                        )
                    pkey = type + "." + id
                    context.rproc[pkey] = True
                    return True
                ...
                if context.debug:
                    log.debug("Empty DELETE response for " + type + " id=" + str(id))

            ## PATCH
            try:
                response = client.get_method(
                    restApiId=restid, resourceId=resid, httpMethod="PATCH"
                )
                if response == []:
                    if context.debug:
                        log.debug("Empty PATCH response for " + type + " id=" + str(id))
                    pkey = type + "." + id
                    context.rproc[pkey] = True
                    return True
                j = response
...
def get_aws_api_gateway_documentation_version(
    type, id, clfn, descfn, topkey, key, filterid
):
        ...
        if id is None:
            ...
            for api in apis:
                try:
                    ...
                    if context.debug:
                        log.debug(
                            f"Error listing documentation versions for API {api['id']}: {e}"
                        )
                    continue
        else:
            ...
            if "/" in id:
                ...
                try:
                    response = client.get_documentation_version(
                        restApiId=rest_api_id, documentationVersion=version
                    )
                    if response:
                        common.write_import(type, id, None)
                except Exception as e:
                    if context.debug:
                        log.debug(f"Error getting documentation version {id}: {e}")
            else:
                if context.debug:
                    log.debug("Must pass restApiId/version for " + type)
        ...
        if id is None:
            ...
            for api in apis:
                try:
                    ...
                    if context.debug:
                        log.debug(
                            f"Error listing gateway responses for API {api['id']}: {e}"
                        )
                    continue
        else:
            ...
            if "/" in id:
                ...
                try:
                    ...
                    if context.debug:
                        log.debug(f"Error getting gateway response {id}: {e}")
            else:
                if context.debug:
                    log.debug("Must pass restApiId/responseType for " + type)
        ...
        if id is None:
            ...
            for api in apis:
                try:
                    ...
                    if context.debug:
                        log.debug(f"Error getting policy for REST API {api['id']}: {e}")
                    continue
        else:
            ...
            try:
                ...
                if context.debug:
                    log.debug(f"Error getting policy for REST API {id}: {e}")
        ...
        if id is None:
            ...
            for plan in plans:
                try:
                    ...
                    if context.debug:
                        log.debug(
                            f"Error listing keys for usage plan {plan['id']}: {e}"
                        )
                    continue
        else:
            ...
            if "/" in id:
                ...
                try:
                    ...
                    if context.debug:
                        log.debug(f"Error getting usage plan key {id}: {e}")
            else:
                if context.debug:
                    log.debug("Must pass usagePlanId/keyId for " + type)
        ...
        if id is None:
            ...
            for api in apis:
                try:
                    ...
                    for page in stage_paginator.paginate(restApiId=api["id"]):
                        for stage in page["item"]:
                            ...
                            if "methodSettings" in stage:
                                for method_path, settings in stage[
                                    "methodSettings"
                                ].items():
                    ...
                    if context.debug:
                        log.debug(f"Error listing stages for API {api['id']}: {e}")
                    continue
        else:
            ...
            if "/" in id:
                ...
                if len(parts) >= 3:
                    ...
                    try:
                        ...
                        if (
                            "methodSettings" in stage
                            and method_path in stage["methodSettings"]
                        ):
                            common.write_import(type, id, None)
                    except Exception as e:
                        if context.debug:
                            log.debug(f"Error getting method settings {id}: {e}")
            else:
                if context.debug:
                    log.debug("Must pass restApiId/stageName/methodPath for " + type)
