...
def get_aws_opensearchserverless_collection(
...
def get_aws_opensearchserverless_security_policy(
    type, id, clfn, descfn, topkey, key, filterid
):
    try:
        config = Config(retries={"max_attempts": 10, "mode": "standard"})
...
def get_aws_opensearchserverless_access_policy(
    type, id, clfn, descfn, topkey, key, filterid
):
    try:
        config = Config(retries={"max_attempts": 10, "mode": "standard"})
...
def get_aws_opensearchserverless_lifecycle_policy(
    type, id, clfn, descfn, topkey, key, filterid
):
    try:
        config = Config(retries={"max_attempts": 10, "mode": "standard"})
        ...
        if id is None:
            ...
            if "/" in id:
                ...
                response = client.batch_get_lifecycle_policy(
                    identifiers=[{"name": name, "type": policy_type}]
                )
                ...
                response = client.batch_get_lifecycle_policy(
                    identifiers=[{"name": id, "type": "retention"}]
                )
                if response.get("lifecyclePolicyDetails"):
...
def get_aws_opensearchserverless_security_config(
    type, id, clfn, descfn, topkey, key, filterid
):
    try:
        config = Config(retries={"max_attempts": 10, "mode": "standard"})
