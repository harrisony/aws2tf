...
def run_terraform_plan_with_progress(
    ...
    try:
        ...
        with tqdm(
            ...
            actual_rate = (
                total_resources / actual_time if actual_time > 0 else estimated_rate
            )
...
def run_terraform_apply_with_progress(tfplan_file, plan_json="plan2.json"):
    ...
    try:
        ...
        with tqdm(
            ...
            actual_rate = (
                actual_count / actual_time if actual_time > 0 else estimated_rate
            )
...
def safe_write_sensitive_file(
    filename: str, content: str, mode: str = "w", base_dir: str = None
) -> None:
    """
...
def secure_terraform_files(directory: str = ".") -> None:
    ...
    for tfvars_file in glob.glob(os.path.join(directory, "*.tfvars")):
        try:
            ...
            if context.debug:
                log.debug(
                    f"Secured {os.path.basename(tfvars_file)} with permissions 0o600"
                )
        except Exception as e:
...
def call_resource(type, id):
    ...
    if type in aws_not_implemented.notimplemented:
        log_warning(
            "Not supported by aws2tf currently: "
            + type
    ...
    if id is not None:
        ...
        if type in needid_dict.aws_needid:
            log_warning(
                "WARNING: "
                + type
                + " can not have null id must pass parameter "
    ...
    try:
        ...
        if module is None:
            ...
            if context.debug:
                log.debug(
                    f"Module not found in registry for clfn={clfn}, will try generic handler"
                )
            sr = False
        else:
    ...
    if not sr:
        try:
            if context.debug:
                log.debug(
                    "calling generic getresource with type="
                    + type
                    + " id="
                    + str(id)
                    + "   clfn="
...
def tfplan1(mymess):
    ...
    if not glob.glob("import*.tf"):
        ...
        log.info(
            "INFO: Confirm the resource type exists in your account: "
            + context.acc
            + " & region: "
            + context.region
        )
        context.tracking_message = (
            "No import*.tf files found for this resource, exiting ...."
        )
        stop_timer()
    ...
    com = (
        "terraform plan -generate-config-out=" + rf + " -out tfplan -json > plan1.json"
    )
    if not context.fast:
        log.info(com)
            ...
            try:
                ...
                try:
                    ...
                    if i != "":
                        ...
                        shutil.move(
                            "aws_*" + i + "*.tf", "notimported/aws_*" + i + "*.tf"
                        )
...
def tfplan3():
    ...
    if context.plan2:
        ...
        com = (
            ...
            + rf
            + " -out tfplan -json > plan2.json"
        )
        if not context.fast:
            log.info(com)

        # Use progress bar for terraform plan
        rout = run_terraform_plan_with_progress(
            com, "Terraform plan (validation)", record_time=True
        )
        ...
        with open("plan2.json", "r") as f:
            for line in f.readlines():
                if '@level":"error"' in line:
                    if (
                        "Error: Conflicting configuration arguments" in line
                        and "aws_security_group_rule." in line
                    ):
                        log.warning(
                        ...
                        try:
                            ...
                            error_detail = error_obj.get("diagnostic", {}).get(
                                "detail", ""
                            )
        ...
        if zerod != 0:
            log.error(
                "-->> plan will destroy resources! - unexpected, is there existing state ?"
            )
        ...
        if zeroc != 0:
            ...
            for pe in planList:
                if (
                    pe["type"] == "planned_change"
                    and pe["change"]["action"] == "update"
                ):
                    nchanges = nchanges + 1
                    ...
                    try:
                        ...
                        if force_destroy_lines and len(force_destroy_lines) == nchanges:
                            ...
                            log.info(
                                "Only force_destroy changes detected (count matches)"
                            )
                        else:
                            all_force_destroy_only = False
                            if context.debug:
                                log.debug(
                                    "force_destroy lines: %d, nchanges: %d",
                                    len(force_destroy_lines),
                                    nchanges,
                                )
                    except Exception as e:
                        all_force_destroy_only = False
                    ...
                    if (
                        ctype == "aws_lb_listener"
                        or ctype == "aws_cognito_user_pool_client"
                        or ctype == "aws_bedrockagent_agent"
                        or ctype == "aws_bedrockagent_agent_action_group"
                        or force_destroy_only
                    ):
                        changeList.append(pe["change"]["resource"]["addr"])
                        log.info(
                            "Planned changes found in Terraform Plan for type: "
                        ...
                        all_force_destroy_only = False
                        log.warning(
                            "Unexpected plan changes found in Terraform Plan for resource: "
                            + str(pe["change"]["resource"]["addr"])
                        )
            if nchanges == nallowedchanges:
                ...
                if all_force_destroy_only and nchanges > 0:
                    log.info(
                        "All changes are force_destroy only - automatically continuing"
                    )
    ...
    if not context.merge:
        if zeroi == awsf:
            ...
            log.info(
                "INFO: import count " + str(zeroi) + " != file counts " + str(awsf)
            )
            if context.workaround == "":
                log.error(
    ...
    if context.merge:
        ...
        if preimpf != stc:
            log.error(
                "Miss-matched previous imports %s and state file resources %s exiting",
                str(preimpf),
                str(stc),
            )
            log.info("exit 029")
...
def wrapup():
    ...
    if context.merge:
        ...
        if not os.path.isfile("plan2.json"):
            log.error(
                "ERROR: Could not find plan2.json, unexpected on merge - exiting ...."
            )
            log.info("exit 033")
            stop_timer()
            exit()
    ...
    if "Error" in str(rout.stderr.decode().rstrip()):
        ...
        if "No changes. Your infrastructure matches the configuration" not in str(
            rout.stdout.decode().rstrip()
        ):
            log.error(errs)
...
def splitf(input_file):
    ...
    if context.debug:
        log.debug("split file: " + input_file)
    ...
    try:
        ...
        if str(response) != "[]":
                ...
                if id is None or filterid == "":  # do it all
                    ...
                    try:
                        if "aws-service-role" in str(item["Path"]):
                            if context.debug:
                                log.debug("Skipping service role " + str(item[key]))
                            continue
                    except:
                        pass
                    ...
                    try:
                        ...
                        with open("boto3-error.err", "a") as f:
                            f.write(
                                "ERROR: getresource TypeError: type="
                                + type
                                + " key="
                                + key
                                + " descfn="
            ...
            if id is not None:
                if context.debug:
                    log.debug(
                        "No " + type + " " + id + " found - empty response (common)"
                    )
                pkey = type + "." + id
                context.rproc[pkey] = True
            else:
                if context.debug:
                    log.debug("No " + type + " found - empty response (common)")
                return True
...
def add_known_dependancy(type, id):
    ...
    if pkey not in context.rdep:
        if context.debug:
            log.debug("add_known_dependancy: " + pkey)
        ...
        if response == []:
            ...
            try:
                ...
                if "apigatewayv2" in str(type):
                    ...
                    if id is not None:
                        if id.startswith("lt-"):
                            for page in paginator.paginate(LaunchTemplateIds=[id]):
                                response.extend(page[topkey])
                        else:
                            for page in paginator.paginate(LaunchTemplateNames=[id]):
                                response.extend(page[topkey])
                    else:
                    ...
                    if id is not None:
                        if "i-" in id:
                            for page in paginator.paginate(InstanceIds=[id]):
                    ...
                    or descfn == "list_fargate_profiles"
                    or descfn == "list_nodegroups"
                    or descfn == "list_identity_provider_configs"
                    or descfn == "list_addons"
                ):
                    for page in paginator.paginate(clusterName=id):
                        response.extend(page[topkey])

                elif descfn == "list_access_keys" and id is not None:
                    for page in paginator.paginate(UserName=id):
                        response.extend(page[topkey])

                elif clfn == "kms" and descfn == "list_aliases" and id is not None:
                    if id.startswith("k-"):
                        id = id[2:]
                    for page in paginator.paginate(KeyId=id):
                        response.extend(page[topkey])
                    return response

                elif clfn == "lambda" and descfn == "list_aliases" and id is not None:
                    for page in paginator.paginate(FunctionName=id):
                        response.extend(page[topkey])
                    return response

                elif clfn == "describe_config_rules" and id is not None:
                    for page in paginator.paginate(ConfigRuleNames=id):
                        response.extend(page[topkey])
                    return response
                ...
                log.error(
                    "ParamValidationError 1 in common.call_boto3: type="
                    + type
                    + " clfn="
                    + clfn
                )
                ...
                if context.debug:
                    log.debug(f"{err=}")
                    log.debug(
                        "calling non paginated fn " + str(descfn) + " id=" + str(id)
                    )
                try:
                    ...
                    if context.debug:
                        log.debug("Non-pag response1=" + str(response1))
                    if id is None:
                        if context.debug:
                            log.debug("id None")
                        response = response1
                        if context.debug:
                            log.debug("Non-pag response no ID =" + str(response))
                    else:  # try a match
                        for j in response1:
                            if id == j[key]:
                                response = [j]
                                if context.debug:
                                    log.debug(
                                        "Non-pag response with ID =" + str(response)
                                    )

                except botocore.exceptions.ParamValidationError as e:
                    log.error(
                        "ParamValidationError 2 in common.call_boto3: type="
                        + type
                        + " clfn="
                        + clfn
                    )
            ...
            if rl == 0:
                if context.debug:
                    log.debug(
                        "** zero response length for "
                        + descfn
                        + " in call_boto3 returning .. []"
                    )
                return []
...
def handle_error(e, frame, clfn, descfn, topkey, id):
    ...
    if exn == "EndpointConnectionError":
        ...
        exn == "ParamValidationError"
        or exn == "ValidationException"
        or exn == "InvalidRequestException"
        or exn == "InvalidParameterValueException"
        or exn == "InvalidParameterException"
    ):
        ...
        if frame.startswith("get_"):
            log.debug(
                ...
                + " "
                + str(id)
                + " check if it exists and what references it - returning"
            )
            ...
            log.debug(
                "NOT FOUND: " + frame + " " + id + " check if it exists - returning"
            )
        return

    elif (
        exn == "ResourceNotFoundException"
        or exn == "EntityNotFoundException"
        or exn == "NoSuchEntityException"
        or exn == "NotFoundException"
        or exn == "LoadBalancerNotFoundException"
        or exn == "NamespaceNotFound"
        or exn == "NoSuchHostedZone"
    ):
        if frame.startswith("get_"):
            log.debug(
                "NOT FOUND: "
        ...
        if clfn == "sqs":
            log.warning(
                "KeyError can not find queue url for "
                + fname
                + " id="
                + str(id)
                + " - returning"
        ...
        if clfn == "ssm":
            log.warning(
                "KeyError can not find ssm document for "
                + fname
                + " id="
                + str(id)
                + " - returning"
        ...
        exn == "AWSOrganizationsNotInUseException"
        or exn == "OrganizationAccessDeniedException"
    ):
        log.warning(
            "NO ORG: "
            + frame
            + " this account doesn't appear to be in an AWS Organisation (or you don't have org permissions) - returning"
        )
        return
        ...
        if "The requested feature is not enabled for this AWS account" in str(exc_obj):
            log.warning(
                descfn + " returned feature not enabled for this account - returning"
            )
            return
        elif "Your account isn't authorized to call this operation" in str(exc_obj):
            log.warning(
                descfn
                + " returned Your account isn't authorized to call this operation - returning"
            )
            return
        log.error(exn)
...
def upload_directory_to_s3():
    ...
    if bret:
        ...
        log.error(
            "Upload to S3 failed - False return from create_bucket_if_not_exists for %s",
            bucket_name,
        )
        return False
...
def trivy_check():
    ...
    try:
        trivy_version = subprocess.check_output(
            ["trivy", "version"], universal_newlines=True
        )
        ver = int(
            "".join(
                filter(str.isdigit, trivy_version.split("\n")[0].split(":")[1].strip())
            )
        )
                ...
                for result in results.get("Results", []):
                    ...
                    if misconfigurations:
                        for misconfig in misconfigurations:
                            ...
                            report.write(
                                json.dumps([resource, description, references]) + "\n"
                            )
            except subprocess.CalledProcessError:
...
def detect_aws_credentials(profile_name=None):
    ...
    try:
                                    ...
                                    active_sso_tokens.append(
                                        {
                                            ...
                                            "region": token_data.get(
                                                "region", "unknown"
                                            ),
                                            "start_url": token_data.get(
                                                "startUrl", "unknown"
                                            ),
                                        }
                                    )
            ...
            try:
                ...
                for profile_section in profiles_to_check:
                    if profile_section in config:
                        ...
                        if (
                            "sso_start_url" in section
                            or "sso_session" in section
                            or "sso_account_id" in section
                            or "sso_role_name" in section
                        ):
                            sso_configured = True
        ...
        if sso_configured and not result["is_sso"]:
            result["details"]["note"] = (
                "SSO is configured but current session is not using SSO"
            )
        elif active_sso_tokens and not result["is_sso"]:
            result["details"]["note"] = (
                "Valid SSO tokens exist but current session is not using SSO"
            )
...
def print_credentials_info(profile_name=None):
    ...
    if info["details"]:
        ...
        if "sso_tokens" in info["details"] and info["is_sso"]:
            ...
            log.info(
                f"Available SSO Tokens (not used by current session): {len(tokens)}"
            )
